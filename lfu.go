...
func (c *LFUCache) GetKeyTTL(key interface{}) (*time.Duration, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	item, ok := c.items[key]
	if ok {
...
func (c *LFUCache) BatchGet(checkExpired bool, keys []interface{}) map[interface{}]interface{} {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (it *lfuItem) GetExpirationDuration(now *time.Time) (*time.Duration, bool) {
	if it.expiration == nil {
		return nil, false
	}
	if now == nil {
